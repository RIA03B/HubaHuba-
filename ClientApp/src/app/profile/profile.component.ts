...
export class ProfileComponent implements OnInit {
  ...
  SaveData(files) {
    this.loading = true;  
    this.clearMessages("ValidationMessage");
    var message = [];
    ...
    if(message.length > 0){
      ...
      var result: CustomerProfiles = [
       ...
       this.http.post(this.baseURL + 'api/UpdateCustomerProfile' , result[0] , httpOptions).subscribe(
        (response2 : headers[]) => {
      ...
      setTimeout(()=>this.messageService.clear("myKey1"),20000);
      if (this.filePhotoName == undefined || this.filePhotoName == null || this.filePhotoName.length <= 0){
      } else {
        this.UploadCustomerPhoto(files);
        setTimeout(()=>this.CPhoto = this.baseURL + "MyStaticFiles/Images/" + this.userID + "/" + files[0].name,5000);
      }
    }
  }
  ...
  getCustomerProfile(){
    this.loading = true;
    this.http.get(this.baseURL + 'api/getCustomerInfo/' + this.userID).subscribe(
        (response : any[]) => {
          this.loading = false;
          this.CustomerInfo = response;
          console.log(this.CustomerInfo);
          if(this.CustomerInfo.length > 0){
            this.FirstName = this.CustomerInfo[0].firstName;
            this.LastName = this.CustomerInfo[0].lastName;
            this.PhoneNumber = this.CustomerInfo[0].phoneNumber;
            this.selectedHairColor = this.haircolors[this.haircolors.findIndex(x => x.name === this.CustomerInfo[0].hairColor)];
            this.selectedSkinColor = this.skincolors[this.skincolors.findIndex(x => x.name === this.CustomerInfo[0].skinColor)];
            this.selectedEyeColor = this.eyecolors[this.eyecolors.findIndex(x => x.name === this.CustomerInfo[0].eyeColor)];
            this.selectedFaceShape = this.faceshapes[this.faceshapes.findIndex(x => x.name === this.CustomerInfo[0].faceShapeID)];
            this.Insecurities = this.CustomerInfo[0].insecurities;
            this.Securities = this.CustomerInfo[0].securities;
            this.FavouriteTvMovie = this.CustomerInfo[0].favTV_Movies;
            this.JobDesc = this.CustomerInfo[0].jobDesc;
            this.AcomplishedFeeling = this.CustomerInfo[0].acomplishedFeeling;
            this.VibeWanted = this.CustomerInfo[0].vibeWanted;
            this.Height = this.CustomerInfo[0].height;
            this.Weight = this.CustomerInfo[0].weight;
            this.CPhoto = this.baseURL + this.CustomerInfo[0].photo;
          }
          console.log(this.selectedEyeColor);
        }, (error) => {})
  }
}